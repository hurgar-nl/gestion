...
projects["root"]		= database.Path() # ipv een pad, anders kom
...
#
#-----------------------------------------------------------------------
def map_to_self( path ):
	print( path )
	print( path.project )
	print( path.userhost )
	# ~ results 	= pathraw_value.split(">:")
	# ~ raw_project	= results[0][1:]
	# ~ print( raw_project ) 
	# ~ path 		= results[1]
	# ~ if database.Userhost().nice in raw_project:
		# ~ return path
	# ~ else:
		# ~ doc = projects_collection.find_one( name = raw_project )
		# ~ if doc != None:
			# ~ new_path = os.path.join( doc[ "root" ], path ) 
			# ~ return new_path
		# ~ else:
			# ~ return path.value
			# ~ raise Exception( f"Cannot map path {path}, no project {raw_project} found for host {database.Userhost().host}")


#-----------------------------------------------------------------------
def map_to_project( path, project ):
	doc = projects_collection.find_one( name = project )
	if doc != None:
		abspath = Path( path )
		return f"<{project}>:path"
	else:
		project = f"default-{database.Userhost().nice}"
		return f"<default-{database.Userhost().nice}>:{path}"
