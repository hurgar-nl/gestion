...
class Blueprint( dict ):
	...
	def __init__( self ):
		...
		if self.__class__ == Blueprint:
			...
			date 		= Date() 
			date.hidden = True
			dict.__setitem__( self, "_modified", date )
	...
	def datatypes( self ):
		return self.keys(  ignore = [ "saved", "id" ] )
	...
	def set_all_values( self ):
		...
		for key in self.keys( ignore = [ "id", "saved" ] ):
			...
			if isinstance( inst, Project ):
				project = inst
				project.value_setter()
			else:
		...
		for inst in instances:
			...
			inst.value_setter()
			# ~ inst.value = f"<{project}>:{path.value}"
...
class Document( Blueprint ):
	...
	def as_datatype( self, attr ):
		if attr in self.keys( ):
			if attr not in  [ "_id", "_saved" ]:
				return Blueprint.__getitem__( self, attr )
			else:
				raise Exception( f"Cannot convert {attr} to a datatype" )
		else:
			raise Exception( f"Document does not contain datatype with the name {attr}" )
...
class Collection( object ):
	...
	def __getattribute__( self, attr ):
		# ~ print( f"__getattribute__( self, {attr} )" )
		if attr in [ "append", "extend", "__setitem__", "clear", "delete_one" ]:
			result = object.__getattribute__( self, attr )
			blueprint 		= self.blueprint
			blueprint[ "_modified"].value_setter()
	...
	def get_last_modification( self ):
		blueprint = self.database[ self.name ].find_one( { "_id": Blueprint.root_id } )
		return blueprint[ "_modified"][ "_raw_value" ]
...
class Datatype( dict ):
	...
	def __init__( self ):
		dict.__init__( self, {	"_raw_value"	: None,
								...
								"_href"			: None,
								"_project"		: "default",
								"_userhost"		: f"{config.USER}@{config.HOST}",
								} )
		self.value_setter()
	...
	@property
	def is_path( self ):
		return issubclass( self.__class__, Path )
	...
	def ameliorate_value( self ):
		return self.value


	def as_html( self, doc:Document ):
		if self.innertext == None:
			return self.ameliorate_value()
		else:
	...
	@property
	def project( self ):
		return dict.__getitem__( self, "_project" )


	@project.setter
	def project( self, value ):
		dict.__setitem__( self, "_project", value )
...
class Date( Integer ):
	...
	def value_setter( self ):
		now = datetime.now()
		self.value = now.timestamp()
	
	
	def ameliorate_value( self ):
		date = datetime.fromtimestamp(1172969203.1)
		return date.strftime('%Y-%m-%d %H:%M:%S')


#-----------------------------------------------------------------------
#
# ~ 'default' zijn lokale bestanden
# ~ dus alle paden zijn absolute
#
#-----------------------------------------------------------------------
class Path( String ):
	def __init__( self, **kwargs ):
	...
	def encoder( self, value ):
		return f"<{self.project}>-<{self.userhost}>:{value}"


	def decoder( self, value ):
		# ~ return value
		user_project, path	= value.split(">:")
		project, userhost	= user_project.split( ">-<" )
